...
**Seeded statuses and their meaning:**

| Code | Terminal | Meaning |
|---|---|---|
| `pending` | No | Default state on client creation — no review has started. |
| `in_progress` | No | An agent has started collecting or verifying documents. |
| `under_review` | No | All documents received; awaiting final decision. |
| `approved` | **Yes** | Client identity verified; eligible for credit products. |
| `rejected` | **Yes** | Client failed verification; cannot proceed. |
| `expired` | **Yes** | Approval lapsed without disbursement within the allowed period. |
| `suspended` | No | Temporarily blocked (e.g. fraud flag); can be re-opened by admin. |
...
Status fields (`kyc_status_id`, `status_id`, `role_id`, `credit_type_id`, `document_type_id`) all reference separate catalogue tables instead of using MySQL `ENUM`. This means new values can be added with a simple `INSERT` instead of an `ALTER TABLE`, and every status carries a human-readable label and metadata (like `is_terminal`).
